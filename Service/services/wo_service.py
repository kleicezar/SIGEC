...
class WorkOrderService:
    ...
    @staticmethod
    def disabled_fields_based_on_situation(situation, workorder):
        situation_object = Situation.objects.filter(id=int(situation)).first() if situation != "0" else workorder.situacao

        form_map_sale = {
            f"{Situation.CLOSURE_LEVEL_OPTIONS[0][0]}": [VendaserviceForm, True],
            f"{Situation.CLOSURE_LEVEL_OPTIONS[1][0]}": [VendaserviceForm, True],
            f"{Situation.CLOSURE_LEVEL_OPTIONS[2][0]}": [VendaserviceForm, True],
            f"{Situation.CLOSURE_LEVEL_OPTIONS[3][0]}": [WorkOrdersReadOnlyForm, False]
        }

        form_map_saleItem = {
            f"{Situation.CLOSURE_LEVEL_OPTIONS[0][0]}": [VendaItemForm, True],
            f"{Situation.CLOSURE_LEVEL_OPTIONS[1][0]}": [WorkOrdersItensReadOnlyForm, False],
            f"{Situation.CLOSURE_LEVEL_OPTIONS[2][0]}": [VendaItemForm, True],
            f"{Situation.CLOSURE_LEVEL_OPTIONS[3][0]}": [WorkOrdersItensReadOnlyForm, False]
        }
        ...
        form_map_serviceItem = {
            f"{Situation.CLOSURE_LEVEL_OPTIONS[0][0]}": [VendaItemserviceForm, True],
            f"{Situation.CLOSURE_LEVEL_OPTIONS[1][0]}": [WorkOrdersItensServicesReadOnly, False],
            f"{Situation.CLOSURE_LEVEL_OPTIONS[2][0]}": [VendaItemserviceForm, True],
            f"{Situation.CLOSURE_LEVEL_OPTIONS[3][0]}": [WorkOrdersItensServicesReadOnly, False]
        }
        return (
            form_map_sale.get(situation_object.closure_level, [VendaserviceForm, True]),
            form_map_saleItem.get(situation_object.closure_level, [VendaItemForm, True]),
            form_map_payments.get(situation_object.closure_level, [PaymentMethodAccountsForm, True]),
            form_map_serviceItem.get(situation_object.closure_level, [VendaItemserviceForm, True])
