...
from sale.forms import VendaForm, VendaItemForm, VendaItemReadOnlyForm, VendaReadOnlyForm
from sale.models import VendaItem

class VendaService:

    @staticmethod
    def disabled_fields_based_on_situation(situation, venda):
        situation_object = Situation.objects.filter(id=int(situation)).first() if situation != "0" else venda.situacao

        form_map_sale = {
            f"{Situation.CLOSURE_LEVEL_OPTIONS[0][0]}": [VendaForm, True],
            f"{Situation.CLOSURE_LEVEL_OPTIONS[1][0]}": [VendaForm, True],
            f"{Situation.CLOSURE_LEVEL_OPTIONS[2][0]}": [VendaForm, True],
            f"{Situation.CLOSURE_LEVEL_OPTIONS[3][0]}": [VendaReadOnlyForm, False]
        }

        form_map_saleItem = {
            f"{Situation.CLOSURE_LEVEL_OPTIONS[0][0]}": [VendaItemForm, True],
            f"{Situation.CLOSURE_LEVEL_OPTIONS[1][0]}": [VendaItemReadOnlyForm, False],
            f"{Situation.CLOSURE_LEVEL_OPTIONS[2][0]}": [VendaItemForm, True],
            f"{Situation.CLOSURE_LEVEL_OPTIONS[3][0]}": [VendaItemReadOnlyForm, False]
        }
        ...
        return (
            form_map_sale.get(situation_object.closure_level, [VendaForm, True]),
